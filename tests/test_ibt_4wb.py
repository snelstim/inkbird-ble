...
def make_notify_payload(
    ...
    return encode(probe1_f10) + encode(probe2_f10) + encode(probe3_f10) + encode(probe4_f10) + b"\x00\x00"
...
def test_ibt_4wb_notify_two_probes_active() -> None:
    ...
    assert DeviceKey("temperature_probe_3", None) not in values
    assert DeviceKey("temperature_probe_4", None) not in values
...
def test_ibt_4wb_notify_all_no_probe() -> None:
    """All probes absent → update is fired but contains no temperature entities."""
    ...
    for probe in range(1, 5):
        assert DeviceKey(f"temperature_probe_{probe}", None) not in values
...
def test_calibration_invalid_probe_raises() -> None:
    ...
    with pytest.raises(ValueError, match="probe_num"):
        import asyncio
        asyncio.run(
            parser.async_ibt_4wb_set_calibration(_ble_device(), {5: 0.0})
        )
